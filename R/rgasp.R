...
rgasp <- function(design, response,trend=matrix(1,length(response),1),zero.mean="No",nugget=0,
                  ...
                  alpha=rep(1.9,dim(as.matrix(design))[2]),multiple_starts=T,lower_bound=T,max_eval=max(30,20+5*dim(design)[2])){
  if (zero.mean=="Yes"){
  ...
  if(is.na(range.par)){
    ...
    cat('Start of the first optimization:','\n')
    ...
    if(multiple_starts){
      cat('Start of the second optimization:','\n')
